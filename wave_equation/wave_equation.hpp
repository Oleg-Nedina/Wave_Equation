 ...
 * - IMPLICIT: Unconditionally stable, requires linear solver (Newmark beta >
 * 0).
 * - EXPLICIT: Conditionally stable (CFL), fast diagonal solve (Newmark beta =
 * 0).
 */
...
public:
  /**
   * Constructor.
   * @param param_file Name of the parameter file (currently unused, parameters
   * are hardcoded for testing).
   */
...
private:
   ...
   * @param time_step The current time step number (used for filename
   * generation).
   */
  ...
  IndexSet locally_relevant_dofs; // Owned + Ghost DoFs needed for
                                  // visualization/gradients

  // --- Constraints ---
  AffineConstraints<double>
      constraints; // Handles hanging nodes and Dirichlet BCs
  ...
  TrilinosWrappers::MPI::Vector
      lumped_mass_matrix; // Diagonal approximation of M
  ...
  // These pointers allow switching between different scenarios (Gaussian, Rain,
  // Ring, etc.) without changing the core solver logic.
