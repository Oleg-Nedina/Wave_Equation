...
WaveEquation<dim>::WaveEquation(const std::string &param_file)
    : mpi_communicator(MPI_COMM_WORLD),
      ...
      // computing_timer: Measures wall-clock time for each section (Assemble,
      // Solve, Output).
      computing_timer(mpi_communicator, pcout, TimerOutput::summary,
                      TimerOutput::wall_times),
      // triangulation: Distributed parallel mesh.
      // MeshSmoothing ensures that when refining, adjacent cells don't differ
      // too much in size.
  ...
  pcout << "Method: " << solver_str << " | dt: " << time_step
        << " | T: " << final_time << " | Scenario: " << scenario_id
        << std::endl;
  pcout << "------------------------------------------" << std::endl;
  ...
  // Strategy Pattern: We assign the correct Function object to the shared
  // pointers. The rest of the code is agnostic to which specific function is
  // being used.
  if (scenario_id == 1) {
    pcout << "Scenario 1: Manufactured Solution (Convergence Test)"
          << std::endl;
    ...
    boundary_function =
        std::make_shared<Functions::ZeroFunction<dim>>(); // Homogeneous
                                                          // Dirichlet
    ...
    initial_forcing_term =
        std::make_shared<RightHandSide_Case2<dim>>(); // reuse F=0
    boundary_function = std::make_shared<Functions::ZeroFunction<dim>>();
 ...
 */
template <int dim> void WaveEquation<dim>::make_grid() {
 ...
 */
template <int dim> double WaveEquation<dim>::compute_error_L2() {
  ...
  VectorTools::integrate_difference(
      dof_handler, U, *exact_solution, difference_per_cell,
  ...
  const double global_error_sq =
      Utilities::MPI::sum(local_error_sq, mpi_communicator);
 ...
 * Sets up the parallel sparsity patterns and resizes all linear algebra
 * objects.
 */
template <int dim> void WaveEquation<dim>::setup_system() {
  ...
  // Note: Dirichlet BCs are handled dynamically in the time loop or via
  // AffineConstraints here.
  constraints.clear();
 ...
 * K_ij = \int \nabla\phi_i \cdot \nabla\phi_j dx
 */
template <int dim> void WaveEquation<dim>::assemble_matrices() {
  ...
  for (const auto &cell : dof_handler.active_cell_iterators()) {
    ...
    if (cell->is_locally_owned()) {
      ...
      // Add local contributions to the global matrices (thread-safe for
      // MPI/Constraints)
...
/**
 * @brief Computes the initial acceleration U''(0) consistent with the strong
 * form.
...
void WaveEquation<dim>::solve_initial_conditions() {
  ...
  TrilinosWrappers::MPI::Vector rhs_initial(locally_owned_dofs,
                                            mpi_communicator);
  rhs_initial.compress(VectorOperation::add);
  ...
  VectorTools::create_right_hand_side(dof_handler, QGauss<dim>(fe.degree + 1),
                                      *initial_forcing_term, rhs_initial);
  ...
  // Identity preconditioner is sufficient because Mass Matrix is
  // well-conditioned
  ...
  pcout << "  Initial acceleration computed. Iterations: "
 ...
 */
template <int dim> void WaveEquation<dim>::assemble_matrix_IMPLICITO() {
...
template <int dim> void WaveEquation<dim>::solve_IMPLICIT() {
  ...
  // algebraic operations (Solver, Matrix-Vector products) to ensure MPI
  // consistency.
  TrilinosWrappers::MPI::Vector U_owned(locally_owned_dofs, mpi_communicator);
  ...
  // Temporary vector for acceleration update (needs ghosts for later use in
  // output)
  TrilinosWrappers::MPI::Vector A_new(locally_owned_dofs, locally_relevant_dofs,
                                      mpi_communicator);
  ...
  while (time < final_time) {
    ...
    // Derived from rearranging the Newmark displacement formula to solve for
    // A_{n+1}. See Eq (4.4) in Project Documentation[cite: 317].
    ...
    VectorTools::interpolate_boundary_values(dof_handler, 0, *boundary_function,
                                             boundary_values);

    // Apply to System Matrix and RHS:
    // Rows corresponding to boundary DoFs are cleared and set to identity
    // diagonal. RHS is set to the boundary value.
    ...
    A_new.add(-(1.0 - 2.0 * beta) / (2.0 * beta),
              A); // Subtract acceleration term
...
template <int dim> void WaveEquation<dim>::compute_lumped_mass_matrix() {
  ...
  const auto range =
      lumped_mass_matrix.local_range(); // pair<global_begin, global_end>

  for (auto i = range.first; i < range.second; ++i) {
    const double m = lumped_mass_matrix(i);
    lumped_mass_matrix(i) = (m > 1e-15) ? (1.0 / m) : 0.0;
  }
 ...
 */
/*template <int dim>
void WaveEquation<dim>::force_ghost_sync(
    const TrilinosWrappers::MPI::Vector &source_owned,
  ...
  // For Trilinos, if operator= fails, we can try re-initializing or simple
  // assignment. But if assignment failed, we check checking constraints
  // distribution.
...
void WaveEquation<dim>::force_ghost_sync(
    const TrilinosWrappers::MPI::Vector &source_owned,
    TrilinosWrappers::MPI::Vector &destination_ghosted) {
  destination_ghosted = source_owned;        // copia owned
  destination_ghosted.update_ghost_values(); // IMPORT dei ghost (fondamentale!)
}
//
  ...
  while (time < final_time) {
    ...
    // Apply Dirichlet BCs to U_{n+1}
    {
      ...
      VectorTools::interpolate_boundary_values(
          dof_handler, 0, *boundary_function, boundary_values);
    ...
    const auto range = A_owned.local_range();
    for (auto i = range.first; i < range.second; ++i)
      A_owned(i) *= lumped_mass_matrix(i);
