...
InputParser::InputParser() { declare_parameters(); }
...
void InputParser::declare_parameters() {
  ...
  prm.enter_subsection("Time Settings");
  {
    ...
    prm.declare_entry("Output frequency", "1", Patterns::Integer(1),
  ...
  {
    prm.declare_entry("Scenario ID", "1", Patterns::Integer(1, 6),
                      "Select test case: 1=Conv, 2=Gauss, 3=Collision, 4=Rain, "
                      "5=Ring, 6=Wall");
    prm.declare_entry("Refinement level", "6", Patterns::Integer(1, 12),
  ...
  prm.enter_subsection("Solver Settings");
  {
    ...
    prm.declare_entry("Newmark Beta", "0.25", Patterns::Double(0.0),
                      "Beta parameter for Newmark scheme.");

    prm.declare_entry("Newmark Gamma", "0.5", Patterns::Double(0.0),
...
void InputParser::parse_parameters(const std::string &filename) {
  ...
  try {
    ...
    std::cerr << "Generating a default 'parameters.prm' file for you."
              << std::endl;
...
unsigned int InputParser::get_refinement_level() const {
  prm.enter_subsection("Scenario");
  int val = prm.get_integer("Refinement level");
  prm.leave_subsection();
  return static_cast<unsigned int>(val);
}
} // namespace WaveEquationProject
